	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<?php
				$id = $item["id"];
				$result = query("SELECT * FROM tb_barang WHERE id = $id");
				?>
				Apakah anda ingin menghapus barang <b class="text-warning"><?= $result[0]["nama_barang"]; ?></b> ?
			</div>
			<div class="modal-footer">
				<button class="btn btn-white" type="button" data-dismiss="modal">Cancel</button>
				<a class="btn btn-danger" href="include/barang/proses/proses-hapus.php?id=<?= $id; ?>">Hapus</a>
			</div>
		</div>
	</div>
</div>